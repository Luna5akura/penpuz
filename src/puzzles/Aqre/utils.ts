import type { AqreClue, AqrePuzzleData } from '../types';

export type AqreCellState = 0 | 1 | 2;

export interface AqreValidationResult {
  valid: boolean;
  message?: string;
  badCells: { r: number; c: number }[];
}

export function createEmptyAqreGrid(width: number, height: number): AqreCellState[][] {
  return Array.from({ length: height }, () => Array(width).fill(0) as AqreCellState[]);
}
...
function parseAqreRoomClues(
  clueData: string,
  regionIds: number[][],
  width: number,
  height: number
) {
  const clues: AqreClue[] = [];
...
export function parseAqreLink(link: string): AqrePuzzleData | null {
  try {
    let dataPart = link.includes('?') ? link.split('?')[1] : link;
    if (dataPart.startsWith('p?')) dataPart = dataPart.slice(2);

    const parts = dataPart.split('/');
    if (parts[0] !== 'aqre' || parts.length < 4) return null;
    ...
    return {
      ...
      height,
      regionIds,
      clues: parseAqreRoomClues(encodedData.slice(expectedCharCount), regionIds, width, height),
    };
  } catch {
    return null;
  }
...
export function getAqreBoundarySegments(regionIds: number[][], width: number, height: number) {
...
function getCluesByRegion(clues: AqreClue[], regionIds: number[][]) {
  const cluesByRegion = new Map<number, AqreClue[]>();
...
export function validateAqre(
  grid: AqreCellState[][],
  puzzle: AqrePuzzleData
): AqreValidationResult {
  ...
  for (let row = 0; row < height; row++) {
    let runStart = 0;
    let runShaded = shaded[row][0];

    for (let col = 1; col <= width; col++) {
      const sameState = col < width && shaded[row][col] === runShaded;
      if (sameState) continue;

      if (col - runStart >= 4) {
        markRunCells(badCells, 'row', row, runStart, col);
        setMessage('横向或纵向都不能出现连续4格或以上同色');
      }

      if (col < width) {
        runStart = col;
        runShaded = shaded[row][col];
      }
    }
  }
  ...
  for (let col = 0; col < width; col++) {
    let runStart = 0;
    let runShaded = shaded[0][col];

    for (let row = 1; row <= height; row++) {
      const sameState = row < height && shaded[row][col] === runShaded;
      if (sameState) continue;

      if (row - runStart >= 4) {
        markRunCells(badCells, 'col', col, runStart, row);
        setMessage('横向或纵向都不能出现连续4格或以上同色');
      }
  ...
  let firstShaded: { row: number; col: number } | null = null;
  let shadedCount = 0;
  for (let row = 0; row < height; row++) {
    for (let col = 0; col < width; col++) {
      if (!shaded[row][col]) continue;
      shadedCount++;
      if (!firstShaded) firstShaded = { row, col };
    }
  }

  if (firstShaded) {
    const visited = Array.from({ length: height }, () => Array(width).fill(false));
    const connectedCount = floodShadedArea(shaded, firstShaded.row, firstShaded.col, visited);

    if (connectedCount !== shadedCount) {
