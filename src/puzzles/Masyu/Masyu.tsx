...
function normalizeMasyuSnapshot(snapshot: unknown, width: number, height: number): MasyuSnapshot {
  const source = snapshot as Partial<MasyuSnapshot> | null | undefined;
  return {
...
export default function MasyuBoard({
  ...
  const createInitialSnapshot = useCallback<() => MasyuSnapshot>(() => ({
    lineEdges: [],
    crossedEdges: [],
    lineEdgeLevels: {},
    crossedEdgeLevels: {},
  ...
  const history = usePuzzleHistory<MasyuSnapshot>(
    initialSnapshot ? normalizeMasyuSnapshot(initialSnapshot, width, height) : createInitialSnapshot(),
    {
      normalizeTrialSnapshot: (trialSnapshot) => ({
        ...normalizeMasyuSnapshot(trialSnapshot, width, height),
        lineEdgeLevels: {},
        crossedEdgeLevels: {},
      }),
  ...
  const normalizedSnapshot = useMemo(
    () => normalizeMasyuSnapshot(snapshot, width, height),
  ...
  const circleMap = useMemo(() => {
    const map = new Map<string, MasyuCell>();
    cells.forEach((rowValues, row) => rowValues.forEach((value, col) => {
      if (value !== 0) map.set(`${row},${col}`, value);
    }));
    return map;
  }, [cells]);
  const hasEdited = canUndo || canRedo || trialActive || trialCheckpointCount > 0;
  const validation = useMemo(
    () => (hasEdited ? validateMasyu(lineEdgeSet, puzzle) : null),
    [hasEdited, lineEdgeSet, puzzle]
  );
  const invalidCellSet = useMemo(
    () => new Set((validation?.badCells ?? []).map((cell) => `${cell.row},${cell.col}`)),
    [validation]
  );
  ...
  useEffect(() => {
    reset(initialSnapshotRef.current ? normalizeMasyuSnapshot(initialSnapshotRef.current, width, height) : createInitialSnapshot());
  ...
  const toggleCrossEdge = useCallback((key: string) => {
    applyChange((currentSnapshot) => {
      const nextSnapshot = normalizeMasyuSnapshot(currentSnapshot, width, height);
  ...
  const handleDocumentPointerMove = useCallback((event: PointerEvent) => {
    ...
    const edgeKey = getMasyuEdgeKey(lastCell.row, lastCell.col, currentCell.row, currentCell.col);
    if (edgeKey) {
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const hitTarget = detectMasyuHitTarget(position.x, position.y, width, height, cellSize);
    if (!hitTarget) return;
        ...
        <div className="grid" style={getBoardGridStyle(BOARD_PADDING, BOARD_PADDING, width, cellSize)}>
          {Array.from({ length: height }, (_, row) =>
            Array.from({ length: width }, (_, col) => {
              const circleValue = circleMap.get(`${row},${col}`);
                  ...
                  {circleValue !== undefined ? (
                    <div
                      ...
                      style={{
                        width: '62%',
                        height: '62%',
                        background: circleValue === 1 ? 'transparent' : woodBoardTheme.border,
